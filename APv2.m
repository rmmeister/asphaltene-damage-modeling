...
% creating TPR curve
Pwf = 4000:1000:9000; % psi
for y = 1:length(Pwf);
   q_TPR(y) = calcFlowrate(depth, rho_o, P_wh, mu, D_tub, Pwf(y), e); % bbl/day
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PSD data
AsphalteneFraction = [0.21 0.165 .125 .09 .07 .05 .04 .03 .015 .01 .03 ...
    .06 .046 .025 .015 0.009 0.007 0.002 0.001 0.0005];
ParticleDiameter = [0.00001 0.02 0.05 0.08 0.11 0.16 0.2 0.21 0.24 0.25 ...
    0.3 0.4 0.56 0.77 1 1.28 1.5 1.75 1.9 2];
...
component_MoleFraction = [0.013887 0.533045 0.047187 0.039397 0.024263 ...
    0.036136 0.058189 0.062541 0.071256 0.021870 0.057985 0.007795 ...
    0.001417 0.019344 0.005688];
component_MolecularWeight = [42.117 16 30 44 74.839 119.657 133.514 ...
    170.318 257.588 265.684 330 453.624 466.771 380.000 475.000];
component_Density = [2 1 2 493 626 670 725 750 1200 1200 1400 1400 1400 ...
    561 1200]; % kg/m3, acc. Wikipedia
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 6. Recalculating final steps for instantaneous flow-rates %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

for i = 1:N
    q2(i) = roots(polyfit(q_TPR, Pwf, 1) - polyfit(q',P_wf(:,i), 1));
end
q2(1) = 14800;
...
for j = 1:N
    ...
    for i = 1:M1-1
        P(i+1,1) = P(1,1) + q2(j)*mu/(7.08*k*h)*log(r(i+1)/rw);
    end
    
A_AP = zeros(M,N);
% calculating the main parameters
    M_RF(j) = q2(j)*(dt/24)*j/v_RF*0.158987294928*1e6; % mole of reservoir fluid
%     Main Loop
    for i = M:-1:1
        A_AP(i,j) = S(P(i,j))*f_trap*M_RF(j)*vA*gamma*6/dH*10.7639; % sqft    
        DOD(i,j) = 1/(1 - (A_AP(i,j)/A_init(i))); % (Step 7)
        ...
        dP_dam(i,j) = q2(j)*mu/1.127/k/A_init(i)*DOD(i,j)*dr;
        if j<N
            P(i,j+1) = P(i,j) - dP_dam(i,j);
            dPs(:,j+1) = P(:,j) - P(:,j+1);
            skin(:,j+1) = dPs(:,j+1)*7.08*k*h/q2(j)/mu;
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
legend('t=0', 't=1.25 hr', 't=2.5 hr', 't=3.75 hr', 't=5 hr', ...
    't=6.25 hr', 't=7.5 hr' ); 
...
% IPR
figure
plot(q, P_wf(:,find(t==1):5:N), 'LineWidth', 2);
title('Inflow Performance Relationship (IPR)');
xlabel('Flow-Rate (bbl/day)');
ylabel('Bottom-Hole Pressure (psi)');
legend('t = 1 hr', 't = 2.25 hr', 't = 3.5 hr', 't = 4.75 hr', ...
    't = 6 hr', 't = 7.25 hr');
hold on
plot(q_TPR, Pwf, '--k', 'LineWidth', 2);
grid on